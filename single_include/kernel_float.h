...
// date: 2026-06-04 17:06:32.719489
// git hash: f9ffc697973ddf6e5c7a58c45885fed5ba3a1054
//================================================================================
...
/**
 * @brief Memory-access policy describing both *what* a `vector_ptr`/`vector_ref` reaches and
 * *how* it reaches it.
 *
 * A policy bundles the storage description that used to be two separate template parameters:
 *  - the underlying storage type `U` (which may be const-qualified for read-only buffers), and
 *  - the assumed pointer `Alignment` in bytes,
 *
 * and it additionally owns the load/store implementation through the static `read`/`write`
 * member templates. `vector_ref::read()` and `vector_ref::write()` forward to these, so a custom
 * policy can change the access strategy (e.g. non-temporal loads, cache hints, bounds checking)
 * without modifying `vector_ref` or `vector_ptr`.
 *
 * A policy is expected to expose:
 *  - `storage_type`        : the underlying element type (possibly `const`),
 *  - `alignment`           : the assumed pointer alignment in bytes,
 *  - `with_alignment<A>`    : an alias rebinding the policy to a new alignment,
 *  - `read<T, N>(const storage_type*) -> vector<T, extent<N>>`,
 *  - `write<T, N>(storage_type*, const vector<T, extent<N>>&)` (omitted for read-only storage).
 *
 * For example, `access_policy<half, 32>` describes a buffer stored as 16-bit `half` that is
 * known to be 32-byte aligned. `access_policy<const half>` describes a read-only `half` buffer
 * aligned to `alignof(half)`.
 *
 * @tparam U The underlying storage type. Use `const U` to describe a read-only buffer.
 * @tparam Alignment The assumed alignment of the pointer in bytes.
 */
template<typename U, size_t Alignment = alignof(U)>
struct access_policy {
    using storage_type = U;
    ...
    /**
     * Produces a policy identical to this one but with a different assumed alignment.
     */
    template<size_t NewAlignment>
    using with_alignment = access_policy<U, NewAlignment>;

    /**
     * Loads ``N`` elements from ``data`` and converts them to user type ``T``.
     */
    template<typename T, size_t N>
    KERNEL_FLOAT_INLINE static vector<T, extent<N>> read(const U* data) {
        vector_storage<U, N> result;
        detail::copy_aligned_impl<U, N, access_alignment>::load(
            result.data(),
            KERNEL_FLOAT_ASSUME_ALIGNED(const U, data, access_alignment));
        return convert<T, N>(result);
    }

    /**
     * Converts ``values`` from user type ``T`` to the storage type and writes ``N`` elements.
     */
    template<typename T, size_t N>
    KERNEL_FLOAT_INLINE static void write(U* data, const vector<T, extent<N>>& values) {
        detail::copy_aligned_impl<U, N, access_alignment>::store(
            KERNEL_FLOAT_ASSUME_ALIGNED(U, data, access_alignment),
            convert_storage<U, N>(values).data());
    }
};

/**
 * Specialization of `access_policy` for read-only (const) storage. It provides ``read`` but
 * deliberately omits ``write``, so that any attempt to store through a read-only policy fails to
 * compile.
 */
template<typename U, size_t Alignment>
struct access_policy<const U, Alignment> {
    ...
    template<size_t NewAlignment>
    using with_alignment = access_policy<const U, NewAlignment>;
    ...
    KERNEL_FLOAT_INLINE static vector<T, extent<N>> read(const U* data) {
        ...
        detail::copy_aligned_impl<U, N, access_alignment>::load(
            result.data(),
            KERNEL_FLOAT_ASSUME_ALIGNED(const U, data, access_alignment));
        return convert_storage<T, N>(result);
    }
};

/**
 * @brief A reference wrapper that allows reading/writing a vector of type `T` and length `N` with
 * optional data conversion.
 *
 * @tparam T The type of the elements as seen from the user's perspective.
 * @tparam N The number of elements in the vector.
 * @tparam PolicyT The access policy describing the underlying storage type and alignment.
 *                 Defaults to `access_policy<T>` (storage type `T`, alignment `alignof(T)`).
 */
template<typename T, size_t N, typename PolicyT = access_policy<T>>
struct vector_ref {
    using policy_type = PolicyT;
    using storage_type = typename PolicyT::storage_type;
    static constexpr size_t alignment = PolicyT::alignment;
    static constexpr size_t access_alignment = PolicyT::access_alignment;
     ...
     * Reads data from the underlying raw pointer, converting it to type `T`.
     *
     * Delegates to `PolicyT::read`.
     *
     * @return vector_type A vector of type vector_type containing the read and converted data.
     */
    KERNEL_FLOAT_INLINE vector_type read() const {
        return PolicyT::template read<value_type, N>(data_);
    }

    /**
     * Writes data to the underlying raw pointer, converting it from the input vector if necessary.
     *
     * Delegates to `PolicyT::write`.
     *
     * @param values The values to be written.
     */
    KERNEL_FLOAT_INLINE void write(const vector_type& values) const {
        PolicyT::template write<value_type, N>(data_, values);
    }
...
/**
 * Specialization for `vector_ref` if the policy's backing storage is const.
 */
template<typename T, size_t N, typename U, size_t Alignment>
struct vector_ref<T, N, access_policy<const U, Alignment>> {
    using policy_type = access_policy<const U, Alignment>;
    using storage_type = const U;
    static constexpr size_t alignment = Alignment;
    static constexpr size_t access_alignment = policy_type::access_alignment;
    ...
    KERNEL_FLOAT_INLINE vector_type read() const {
        return policy_type::template read<value_type, N>(data_);
    }
...
#define KERNEL_FLOAT_VECTOR_REF_ASSIGN_OP(OP, OP_ASSIGN)              \
    template<typename T, size_t N, typename PolicyT, typename V>      \
    KERNEL_FLOAT_INLINE vector_ref<T, N, PolicyT> operator OP_ASSIGN( \
        vector_ref<T, N, PolicyT> ptr,                                \
        const V                                                       \
        & value) {                                                    \
        ptr.write(ptr.read() OP value);                               \
        return ptr;                                                   \
...
template<typename T, size_t N, typename PolicyT>
struct into_vector_impl<vector_ref<T, N, PolicyT>> {
    using value_type = T;
    using extent_type = extent<N>;

    KERNEL_FLOAT_INLINE
    static vector_storage<value_type, N> call(const vector_ref<T, N, PolicyT>& reference) {
        return reference.read();
    }
};

template<typename T, size_t N, typename PolicyT>
struct is_vector_impl<vector_ref<T, N, PolicyT>> {
    static constexpr bool value = true;
};
 ...
 * A wrapper for a pointer that enables vectorized access and supports type conversions..
 *
 * The `vector_ptr<T, N, PolicyT>` type is designed to function as if its a `vec<T, N>*` pointer, allowing of reading
 * and writing `vec<T, N>` elements. However, the actual type of underlying storage is a pointer of type
 * `PolicyT::storage_type*`, where automatic conversion is performed between `T` and the storage type when
 * reading/writing items.
 *
 * For example, a `vector_ptr<double, N, access_policy<half>>` is useful where the data is stored in low precision
 * (here 16 bit) but it should be accessed as if it was in a higher precision format (here 64 bit).
 *
 * @tparam T The type of the elements as viewed by the user.
 * @tparam N The alignment of T in number of elements.
 * @tparam PolicyT The access policy describing the underlying storage type and pointer alignment.
 *                 Defaults to `access_policy<T, sizeof(T) * N>`.
 */
template<typename T, size_t N, typename PolicyT = access_policy<T, sizeof(T) * N>>
struct vector_ptr {
    using policy_type = PolicyT;
    using storage_type = typename PolicyT::storage_type;
    static constexpr size_t alignment = PolicyT::alignment;
    ...
    static constexpr size_t offset_alignment = detail::gcd(alignment, sizeof(storage_type) * N);
    using offset_policy = typename PolicyT::template with_alignment<offset_alignment>;
    ...
    /**
     * Constructs a vector_ptr from another vector_ptr with potentially different alignment. This constructor
     * only allows conversion if the alignment of the source is greater than or equal to the alignment of the target.
     */
    template<
        typename T2,
    ...
    KERNEL_FLOAT_INLINE vector_ptr(vector_ptr<T2, N2, access_policy<storage_type, A2>> p) :
        data_(p.get()) {}
     ...
     * Accesses a reference to a vector at a specific index with optional alignment considerations.
     *
     * @tparam K The number of elements in the vector to access, defaults to N.
     * @param index The index at which to access the vector.
     */
    template<size_t K = N>
    KERNEL_FLOAT_INLINE vector_ref<value_type, K, offset_policy> at(size_t index) const {
        return vector_ref<value_type, K, offset_policy> {data_ + index * N};
    }

    /**
     * Shorthand for `at(index)`.
     */
    KERNEL_FLOAT_INLINE vector_ref<value_type, N, offset_policy> operator[](size_t index) const {
        return at(index);
    }
...
/**
 * Specialization for `vector_ptr` if the policy's backing storage is const.
 */
template<typename T, size_t N, typename U, size_t Alignment>
struct vector_ptr<T, N, access_policy<const U, Alignment>> {
    using policy_type = access_policy<const U, Alignment>;
    using storage_type = const U;
    static constexpr size_t alignment = Alignment;

    static constexpr size_t offset_alignment = detail::gcd(Alignment, sizeof(U) * N);
    using offset_policy = typename policy_type::template with_alignment<offset_alignment>;
    ...
    template<
        ...
        size_t A2,
        enable_if_t<detail::is_alignment_divisible(A2, Alignment), int> = 0>
    KERNEL_FLOAT_INLINE vector_ptr(vector_ptr<T2, N2, access_policy<const U, A2>> p) :
        data_(p.get()) {}
    ...
    template<
        ...
        size_t A2,
        enable_if_t<detail::is_alignment_divisible(A2, Alignment), int> = 0>
    KERNEL_FLOAT_INLINE vector_ptr(vector_ptr<T2, N2, access_policy<U, A2>> p) : data_(p.get()) {}
...
template<typename T, size_t N, typename U, size_t A>
KERNEL_FLOAT_INLINE vector_ptr<T, N, access_policy<U, detail::gcd(A, N * sizeof(U))>>
operator+(vector_ptr<T, N, access_policy<U, A>> p, size_t i) {
    return vector_ptr<T, N, access_policy<U, detail::gcd(A, N * sizeof(U))>> {p.get() + i * N};
}
...
template<typename T, size_t N, typename U, size_t A>
KERNEL_FLOAT_INLINE vector_ptr<T, N, access_policy<U, detail::gcd(A, N * sizeof(U))>>
operator+(size_t i, vector_ptr<T, N, access_policy<U, A>> p) {
    return p + i;
}
...
/**
 * Creates a `vector_ptr<T, N, access_policy<U, sizeof(U) * N>>` from a raw pointer `U*`.
 *
 * @tparam T The type of the elements as viewed by the user.
 * @tparam N The vector size in number of elements.
 * @tparam U The type of the elements pointed to by the raw pointer.
 */
template<typename T, size_t N = 1, typename U>
KERNEL_FLOAT_INLINE vector_ptr<T, N, access_policy<U, sizeof(U) * N>> make_vec_ptr(U* ptr) {
    return vector_ptr<T, N, access_policy<U, sizeof(U) * N>> {ptr};
}
...
template<typename T, typename U = T>
using scalar_ptr = vector_ptr<T, 1, access_policy<U, alignof(U)>>;

#if defined(__cpp_deduction_guides)
template<typename T>
vector_ptr(T*) -> vector_ptr<T, 1, access_policy<T, alignof(T)>>;

template<typename T>
vector_ptr(const T*) -> vector_ptr<T, 1, access_policy<const T, alignof(T)>>;
